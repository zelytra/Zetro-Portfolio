<template>
  <div class="input-wrapper">

    <div class="wrapper input" @click="openOption()">
      <div class="selected-values">
        <p v-for="option of checked" v-if="checked.length > 0">
          {{ option.max >= 99999 ? option.min + '€ +' : option.min + '€ -' }}
          {{ option.max >= 99999 ? '' : option.max + '€' }}</p>
        <span v-else>{{ placeHolder }}</span>
      </div>
      <img src="@assets/icons/small_arrow.svg" alt="arrow" :class="{open:toggleOption}"/>
    </div>

    <transition>
      <div class="wrapper options" v-if="toggleOption" v-click-outside="() => closeOption()"
           @mouseleave="closeOption()">
        <div class="option"
             v-for="option of options"
             :class="{selected:checked.includes(option)}"
             @click="forceCheckUpdate(option)">
          <input type="checkbox"
                 :id="option+'-input'"
                 v-model="checked"
                 :value="option"
          />
          <label>{{ option.max >= 99999 ? option.min + '€ +' : option.min + '€ -' }}
            {{ option.max >= 99999 ? '' : option.max + '€' }}</label>
        </div>
      </div>
    </transition>
  </div>
</template>

<script setup lang="ts">
import {ref} from "vue";
import {Range} from "@/object/UserProfile";

const checked = ref<Range[]>([])
const toggleOption = ref<boolean>(false)
const emits = defineEmits(['selectChange'])
const options = [
  {
    min: 10,
    max: 50
  },
  {
    min: 50,
    max: 150
  },
  {
    min: 150,
    max: 300
  }, {
    min: 300,
    max: 99999999
  }
]

defineProps({
  placeHolder: String
})

function forceCheckUpdate(option: Range) {
  if (checked.value.includes(option)) {
    checked.value = []
  } else {
    checked.value = [option]
  }
  emits('selectChange', checked.value[0])
}

function openOption() {
  toggleOption.value = true
}

function closeOption() {
  toggleOption.value = false;
}
</script>

<style scoped lang="scss">
@import "@assets/style.scss";

.input-wrapper {
  position: relative;
  width: 240px;

  .wrapper {
    border: 1px solid var(--main);
    border-radius: 8px;
    padding: 10px 19px;
    background: var(--input-background);

    &.input {
      display: flex;
      align-content: center;
      justify-content: space-between;

      .selected-values {
        overflow: hidden;
        display: flex;
        align-items: center;
        gap: 4px;
        max-width: 90%;

        :not(:last-child):nth-child(n):after {
          content: ',';
        }

        p {
          white-space: nowrap;
        }

        span {
          opacity: 0.5;
        }
      }

      img {
        width: 12px;

        &.open {
          transform: rotate(180deg);
        }
      }
    }

    &.options {
      top: 46px;
      left: 0;
      right: 0;
      position: absolute;
      z-index: 99;
      width: auto;
      display: flex;
      flex-direction: column;
      gap: 6px;

      @media screen and (max-width: $responsive-break-point) {
        top: unset;
        position: relative;
        margin-top: 6px;
      }

      .option {
        padding: 10px;
        border-radius: 8px;
        display: flex;
        align-items: center;
        gap: 6px;
        filter: brightness(0.8);

        &:hover {
          filter: brightness(1);
        }

        input[type="checkbox"] {
          appearance: none;
          border: 2px solid #C9CED6;
          border-radius: 4px;
          width: 20px;
          height: 20px;
          display: flex;
          align-items: center;
          justify-content: center;

          &:before {
            content: '';
            width: 10px;
            transform: scale(0);
            height: 10px;
            background: var(--revert-text);
            clip-path: polygon(14% 44%, 0 65%, 50% 100%, 100% 16%, 80% 0%, 43% 62%);
            transform-origin: bottom left;
          }

          &:checked {
            background: var(--main);
            border: 2px solid var(--main);

            &:before {
              transform: scale(1);
            }
          }
        }

        label {
          color: var(--primary-text);
        }

        &.selected {
          label {
            color: var(--main);
          }
        }
      }
    }
  }
}

.v-leave-active, .v-enter-active {
  transition: 0.2s ease;
}

.v-enter-from,
.v-leave-to {
  opacity: 0;
}
</style>
